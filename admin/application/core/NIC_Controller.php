...
class NIC_Controller extends CI_Controller {
	...
	public function __construct(){
		...
		//$this->output->set_header("Content-Security-Policy: default-src 'self' 'unsafe-inline' 'unsafe-eval'; img-src 'self' data:; script-src 'self' 'unsafe-inline' 'unsafe-eval'; frame-src 'self' 'unsafe-inline' 'unsafe-eval'; object-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; connect-src 'self' 'unsafe-inline' 'unsafe-eval' http://localhost:8080; font-src 'self' 'unsafe-inline' 'unsafe-eval';");

		$this->output->set_header("Content-Security-Policy: 
		default-src 'self' 'unsafe-inline' 'unsafe-eval' https://code.highcharts.com; 
		img-src 'self' data:; 
		script-src 'self' 'unsafe-inline' 'unsafe-eval' https://code.highcharts.com https://code.highcharts.com/modules/; 
		frame-src 'self' 'unsafe-inline' 'unsafe-eval'; 
		object-src 'self' 'unsafe-inline' 'unsafe-eval'; 
		style-src 'self' 'unsafe-inline'; 
		connect-src 'self' 'unsafe-inline' 'unsafe-eval' http://localhost:8080; 
	...
	public function check_otp_priviledge()
	{
	// if($this->session->userdata('login_id') != NULL){
	// 	redirect('admin/dashboard', 'location');
	// }
	if($this->session->userdata('login_code') != NULL && $this->session->userdata('secretotp'))
	{
		redirect('admin/dashboard', 'location');
	}
	}
