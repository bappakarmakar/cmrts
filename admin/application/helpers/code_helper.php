...
function get_cp_full_age($entry_date, $cp_dob){
...
// Scheduler Module code BY Soumen 22_03_2025 Start
function Get_CP_Homevisit_state_scheduler($cp_id_fk){
    $ci=& get_instance();
    $ci->load->database();
    $ci->db->select('sl_no,hv_status,incident_id_fk,active_status');
...
//home enquiry by soumen
function cp_status_he($current_status, $cp_id_fk, $cp_age)
{
    $status = '';
    $x = Get_CP_Homevisit_state_scheduler($cp_id_fk);
    if($current_status == 3 )
    {
        if(!empty($x))
        {
            if($x['hv_status'] == 0 )
            {
                $status = '<p style="padding: 0;margin: 0;color: #c2700c;font-weight: bold;">Saved As Draft</p>';;
            }
            else if($x['hv_status'] == 1 )
            {
                $status = '<p style="padding: 0;margin: 0;color: #0c5bc2;font-weight: bold;">Saved</p>';
            }
            else if($x['hv_status'] == 2 )
            {
                $status = '<p style="padding: 0;margin: 0;color: #078cb5;font-weight: bold;">Forwarded</p>';
            }
            else if($x['hv_status'] == 3 )
            {
                $status = '<p style="padding: 0;margin: 0;color: #125e00;font-weight: bold;">Home Enquiry is published</p>';
            }
            else if($x['hv_status'] == 4 )
            {
                $status = '<p style="padding: 0;margin: 0;color: #c41704;font-weight: bold;">Reverted</p>';
            }            
        }
        else
        {
...
// Follow Up Visits by soumen
function Get_CP_Follow_Up_visit_state_fuv( $scheduler_id){
    //echo $cp_id_fk."<br>".$scheduler_id;
    $ci=& get_instance();
    $ci->load->database();
    $ci->db->select('sl_no,fv_status,incident_id_fk,scheduler_id_fk');
    $ci->db->from('cm_follow_up_visit_details');
    $ci->db->where('active_status' , 1);
    $ci->db->where('scheduler_id_fk' , $scheduler_id);
    $row = $ci->db->get()->row_array();
...
// Follow up visits count by soumen
function Get_CP_Followup_published_Count_scheduler($scheduler_id){
    $ci=& get_instance();
    $ci->load->database();
    $ci->db->select('sl_no');
    $ci->db->from('cm_follow_up_visit_details');
...
// Follow Up Visits by soumens
function cp_status_fuv($current_status, $cp_id_fk, $cp_age, $scheduler_id)
{
    $status = '';
    $x = Get_CP_Follow_Up_visit_state_fuv($scheduler_id);
    if($current_status == 3)
    {
      if(!empty($x))
        {
            ...
            else if($x['fv_status'] == 1)
            {
                    $status = '<p style="padding: 0;margin: 0;color: #0c5bc2;font-weight: bold;">Saved</p>';
            }
            else if($x['fv_status'] == 2)
            {
                    $status = '<p style="padding: 0;margin: 0;color: #078cb5;font-weight: bold;">Forwarded</p>';
            }
            else if($x['fv_status'] == 3)
            {
                $status = '<p style="padding: 0;margin: 0;color: #125e00;font-weight: bold;">Follow-Up is published</p>';
            }

            else if($x['fv_status'] == 4)
            {
                $status = '<p style="padding: 0;margin: 0;color: #c41704;font-weight: bold;">Reverted</p>';
            }
        }
        else
        {
            if($cp_age >= 18)
            {
                $status = "Follow-Up Visit Pending";
            }
            else{
                $status =  "Follow-Up Visit Pending";
            }
...
function get_min_calculated_date($incident_id_pk,$cp_type) { //FUNCTION BY SOUMEN TO GET SCHEDULER MINIMUM DATE
    $CI =& get_instance();
    $CI->load->database();
    // echo $cp_type;die;
    $CI->db->select('MIN(calculated_date) AS min_date');
    $CI->db->from('cm_follow_up_visit_minor_scheduler');
    $CI->db->where('incident_id', $incident_id_pk);
    $CI->db->where('cp_type', $cp_type);
    $CI->db->where('active_status', 0);
...
// Scheduler Module code BY Soumen 22_03_2025 END
...
// --------------- SCHEDULER NEW SCREEN HELPER FUNCTIONS 18-03-2025 By Soumen ----------------
function Get_he_details($scheduler_id){
    $ci=& get_instance();
    $ci->load->database();
    $ci->db->select('*');
    $ci->db->from('cm_incident_report_home_visit');
...
function Get_fuv_details($scheduler_id){
    $ci=& get_instance();
    $ci->load->database();
    $ci->db->select('*');
    $ci->db->from('cm_follow_up_visit_details');
...
function full_he_age_echo($scheduler_id, $cp_dob){
    $data = Get_he_details($scheduler_id);
    // print_r($entry_date);die;
    if($data && $cp_dob){
        $entry_date = $data['home_enquiry_date']; 
        // Convert dates to DateTime objects
        $dob = new DateTime($cp_dob);
        $entry_date = new DateTime($entry_date);
        // Calculate the difference
        $age = $dob->diff($entry_date);
       echo "<strong>" . $age->y . "</strong> years <strong>" . $age->m . "</strong> months <strong>" . $age->d . "</strong> days";
    }else{
        echo "-";
    }
}

function full_fuv_age_echo($scheduler_id, $cp_dob){
    $data = Get_fuv_details($scheduler_id);
    if($data && $cp_dob){
        $entry_date = $data['followup_date'];
        // Convert dates to DateTime objects
        $dob = new DateTime($cp_dob);
        $entry_date = new DateTime($entry_date);
        // Calculate the difference
...
function full_he_fuv_age_in_days($scheduler_id, $cp_dob){
    $he_data = Get_he_details($scheduler_id);
    
    $fu_data = Get_fuv_details($scheduler_id);
     
    if(isset($he_data)){
        $he_entry_date = $he_data['home_enquiry_date'];
        if ($he_entry_date && $cp_dob) {
            $dob = new DateTime($cp_dob);
            $he_entry_date = new DateTime($he_entry_date);
            $he_age_diff = $dob->diff($he_entry_date);
            return $he_age_diff->days;
        } else {
            return ""; 
        }
    }elseif(isset($fu_data)){
         $fuv_entry_date = $fu_data['followup_date'];
        if ($fuv_entry_date && $cp_dob) {
            $dob = new DateTime($cp_dob);
            $fuv_entry_date = new DateTime($fuv_entry_date);
            $fu_age_diff = $dob->diff($fuv_entry_date);
            return $fu_age_diff->days;
        } else {
            return ""; 
        }
    }else{
...
// ----------------- SCHEDULER NEW SCREEN 18-03-2025 END By Soumen ------------------

// ---------------- SCHEDULER MIS FUNCTION 28-04-2025 START By Soumen ---------------
...
// -------------- SCHEDULER MIS FUNCTION 28-04-2025 END By Soumen ---------------------
