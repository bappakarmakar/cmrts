...
<div class="content-wrapper">
   <section class="content-header">
      <h1>User Change Request List (received)</h1>
      <ol class="breadcrumb">
   ...
   <section class="content">
      ...
      <div class="box bottom-box">
            <!-- <a href="<?php echo base_url()?>admin/user_change/user_change" class="btn btn-primary" style="margin-top: 8px; float: right; margin-right: 10px;margin-bottom: 12px;">
                                      ...
                                      ?>
                                        <ul class="dropdown-menu action_menu" role="menu" aria-labelledby="menu1" style="margin-left: -83px;">
                                            <li role="presentation">
                                               <a class="" onclick="Approve_request('<?php echo base64_encode($value['sl_no']); ?>', '<?php echo base64_encode($value['stake_holder_login_id_fk']); ?>')">
                                               <i class="fa fa-forward"></i> Approve</a>
                                               </a>
                                            </li>
                                            <li role="presentation">
                                               <a class="" onclick="Revert_request('<?php echo base64_encode($value['sl_no']); ?>', '<?php echo base64_encode($value['stake_holder_login_id_fk']); ?>')">
                                               <i class="fa fa-backward"></i> Reject</a>
                                            </li>
                                               </a>
                                        </ul>
                                        <?php 
        ...
        if(isConfirm) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    swal("Approved!", "approve success", "success");
                    setTimeout(function(){
                        window.location.reload();
                    }, 2000);
                },
                error: function(xhr, status, error) {
                    swal("Error", "An error occurred while approving", "error");
                    console.error(xhr.responseText);
                }
            });
        } else {
            swal("Cancelled", "approve cancel!", "error");
            setTimeout(function(){
                window.location.reload();
            }, 1500);
        } 
...
<script type="text/javascript">
 function Revert_request(sl_no,stake_holder_login_id_fk) {
    // First SweetAlert for confirmation
    // alert(sl_no+"--------"+stake_holder_login_id_fk);
    swal({
        ...
        if (isConfirmed) {
            ...
            swal({
                ...
                var url = '<?php echo base_url()?>admin/user_change/user_change_list/revert_request/';
                $.ajax({
                    url: url,
                    method: "GET",
                    data: {'sl_no': sl_no,'stake_holder_login_id_fk': stake_holder_login_id_fk, 'reason': $('#revert').val()},
                    dataType: "JSON",
                    success: function(response) {
                        if (response.status === 'success') {
                            swal("Rejected!", "Reject success", "success");
                            setTimeout(function() {
                                window.location.reload();
                            }, 2000);
                        } else {
                    ...
                    error: function(xhr, status, error) {
                        swal("Error", "An error occurred while rejecting", "error");
                        console.error(xhr.responseText);
                    }
                });
            });
