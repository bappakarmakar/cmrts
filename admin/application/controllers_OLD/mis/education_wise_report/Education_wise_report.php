...
class Education_wise_report extends NIC_Controller {


  private $district_wise = [
    'field_selection' => "cmir.district as unique_id, cmir.district as district_id_pk, district_location_master_description(cmir.district) as name,",
    'group_by' => "cmir.district",
    'order_by' => "district_location_master_description(cmir.district)"
    // 'order_by' => "cmir.district"
  ]; 

  private $block_wise = [
      'field_selection' => "cmir.block as unique_id, block_location_master_description(cmir.block) as name,",
      'group_by' => "cmir.block",
      'order_by' => "block_location_master_description(cmir.block)"
      // 'order_by' => "cmir.block"
  ];
  ...
  public function index()
  {
    ...
    if (strtoupper($this->input->server("REQUEST_METHOD")) == strtoupper('GET'))
    {
        ...
        if(empty($from_date)){
            ...
            $data['force_view'] = 1;
            $data['from_date'] = $from_date = date('Y-m-d', strtotime(' - 30 days'));
    ...
    // DIE;
    //  echo "</pre>";
    $data['report_result'] = $this->Education_wise_mis_model->get_education_wise_child_dtls($data);

    $this->load->view($this->config->item('theme').'mis/education_wise_report/education_wise_report_view', $data);
  }
  ...
  public function download_excel()
  {
    // echo "<pre>"; print_r($_GET);die;
    $filename = '';
    $title = 'Education-Wise analysis of contracting Parties';
    $data['delete_status'] = 0;
    $data['current_status'] = 3;
    ...
    $list_for_name = null;
      ...
      if($data['segregate'] == 'district')
      {
        ...
        $data['group_by'] = $this->district_wise['group_by'];
        $data['order_by'] = $this->district_wise['order_by'];
        // $title = "MIS_education_taken_Districtwise";
        $title .= ' (State-wide)';
        $filename = "MIS_Education_Wise_State_Wide";
        //title name 
      ...
      elseif ($data['segregate'] == 'block') 
      {
        ...
        // $title = "MIS_education_taken_Blockwise";
        $title .= ' for '.$list_for_name;
        $filename = "MIS_Education _Wise_".$list_for_name;
      }
      elseif ($data['segregate'] == 'ward_gp') 
      {
        ...
        // $title = "MIS_education_taken_WardGP_wise";
      ...
      $report_result = $data['report_result'] = $this->Education_wise_mis_model->get_education_wise_child_dtls($data);
    ...
    $sheet->getStyle('A1:R2')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('33ccff');
    $sheet->getStyle('A3:R4')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('ffcc00');

    $sheet->getStyle('A1:R1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('33ccff');
    ...
    $sheet->setCellValue('C3', 'Not Reported');
    $sheet->setCellValue('C4', 'Female');
    $sheet->setCellValue('D4', 'Male');

    $sheet->setCellValue('E3', 'Never attended school');
    $sheet->setCellValue('E4', 'Female');
    $sheet->setCellValue('F4', 'Male');

    $sheet->setCellValue('G3', 'Up to Class V');
    $sheet->setCellValue('G4', 'Female');
    $sheet->setCellValue('H4', 'Male');

    $sheet->setCellValue('I3', 'Class V to VIII');   
    $sheet->setCellValue('I4', 'Female');
    $sheet->setCellValue('J4', 'Male');

    $sheet->setCellValue('K3', 'Class IX - X');  
    $sheet->setCellValue('K4', 'Female');
    $sheet->setCellValue('L4', 'Male');

    $sheet->setCellValue('M3', 'Class XI - XII');  
    $sheet->setCellValue('M4', 'Female');
    $sheet->setCellValue('N4', 'Male'); 

    $sheet->setCellValue('O3', 'Above XII');  
    $sheet->setCellValue('O4', 'Female');
    $sheet->setCellValue('P4', 'Male'); 

    $sheet->setCellValue('Q3', 'Total');  
    $sheet->setCellValue('Q4', 'Female');
    $sheet->setCellValue('R4', 'Male'); 
    ...
    foreach ($data['report_result'] as $value)
    {
      ...
      $sheet->setCellValue('C' . $rows, ($value['female_count_not_report'] != 0)?($value['female_count_not_report']):0);
      $sheet->setCellValue('D' . $rows, ($value['male_count_not_report'] != 0)?($value['male_count_not_report']):0 );

      $sheet->setCellValue('E' . $rows, ($value['female_count_never_attended'] != 0)?($value['female_count_never_attended']):0 );
      $sheet->setCellValue('F' . $rows, ($value['male_count_never_attended'] != 0)?($value['male_count_never_attended']):0);

      $sheet->setCellValue('G' . $rows, ($value['female_count_upto_5'] != 0)?($value['female_count_upto_5']):0 );
      $sheet->setCellValue('H' . $rows, ($value['male_count_upto_5'] != 0)?($value['male_count_upto_5']):0);

      $sheet->setCellValue('I' . $rows, ($value['female_count_5_to_8'] != 0)?($value['female_count_5_to_8']):0 );
      $sheet->setCellValue('J' . $rows, ($value['male_count_5_to_8'] != 0)?($value['male_count_5_to_8']):0);

      $sheet->setCellValue('K' . $rows, ($value['female_count_9_to_10'] != 0)?($value['female_count_9_to_10']):0);
      $sheet->setCellValue('L' . $rows, ($value['male_count_9_to_10'] != 0)?($value['male_count_9_to_10']):0);

      $sheet->setCellValue('M' . $rows, ($value['female_count_11_to_12'] != 0)?($value['female_count_11_to_12']):0 );
      $sheet->setCellValue('N' . $rows, ($value['male_count_11_to_12'] != 0)?($value['male_count_11_to_12']):0);

      $sheet->setCellValue('O' . $rows, ($value['female_count_above_12'] != 0)?($value['female_count_above_12']):0 );
      $sheet->setCellValue('P' . $rows, ($value['male_count_above_12'] != 0)?($value['male_count_above_12']):0);

      $sheet->setCellValue('Q' . $rows, ($value['total_female_count'] != 0)?($value['total_female_count']):0 );
      $sheet->setCellValue('R' . $rows, ($value['total_male_count'] != 0)?($value['total_male_count']):0 );
    ...
    if($total_row>1)
    {
      ...
      $sheet->setCellValue('C' . $total_row,array_sum(array_column($report_result, 'female_count_not_report')));
      $sheet->setCellValue('D' . $total_row,array_sum(array_column($report_result, 'male_count_not_report')));

      $sheet->setCellValue('E' . $total_row,array_sum(array_column($report_result, 'female_count_never_attended')));
      $sheet->setCellValue('F' . $total_row,array_sum(array_column($report_result, 'male_count_never_attended')));

      $sheet->setCellValue('G' . $total_row,array_sum(array_column($report_result, 'female_count_upto_5')));
      $sheet->setCellValue('H' . $total_row,array_sum(array_column($report_result, 'male_count_upto_5')));

      $sheet->setCellValue('I' . $total_row,array_sum(array_column($report_result, 'female_count_5_to_8')));
      $sheet->setCellValue('J' . $total_row,array_sum(array_column($report_result, 'male_count_5_to_8')));

      $sheet->setCellValue('K' . $total_row,array_sum(array_column($report_result, 'female_count_9_to_10')));
      $sheet->setCellValue('L' . $total_row,array_sum(array_column($report_result, 'male_count_9_to_10')));

      $sheet->setCellValue('M' . $total_row,array_sum(array_column($report_result, 'female_count_11_to_12')));
      $sheet->setCellValue('N' . $total_row,array_sum(array_column($report_result, 'male_count_11_to_12')));


      $sheet->setCellValue('O' . $total_row,array_sum(array_column($report_result, 'female_count_above_12')));
      $sheet->setCellValue('P' . $total_row,array_sum(array_column($report_result, 'male_count_above_12')));


      $sheet->setCellValue('Q' . $total_row,array_sum(array_column($report_result, 'total_female_count')));
      $sheet->setCellValue('R' . $total_row,array_sum(array_column($report_result, 'total_male_count')));
