...
class Police_case_form extends NIC_Controller {
  ...
  public function date_less_check($fir_date)
  {
    ...
    $incident_date = $this->input->post('incident_date');
    if(empty($fir_date)){
      $this->form_validation->set_message('date_less_check', 'The Date field is required');
      ...
      if(strtotime($fir_date)<strtotime($incident_date))
      {
        $this->form_validation->set_message('date_less_check', 'The date should not be less then incident date: '. $this->input->post('incident_date'));
        return false;
      }
        return true;
    }
  ...
  public function date_less_check1($fir_date)
  {
