...
class Forgot_password_form extends NIC_Controller {
         ...
         if ($this->form_validation->run() == TRUE)
         {
            ...
            if($check_result)
            {
               if($check_result[0]['mobile_no'])
               {
                  ...
                  $body="WCD&SW Department, GoWB - Your OTP Verification Code is ".$random;
                  $temp_id="1007161190860534251";
                  $body_len=strlen($body);
                     $data['insert_otp_details'] = array(
  ...
  public function otp_check()
  {
      ...
      if($this->input->method(TRUE) == 'POST' && $this->session->userdata('fgot_otp_status')==1 && $this->session->userdata('otp_status')!=1)
      {
         $this->form_validation->set_error_delimiters('<div class="text-danger">', '</div>');
         $config = array(
          array(
     ...
     if($this->input->method(TRUE) == 'POST')
     {
         ...
         if($this->form_validation->run() == TRUE)
         {
            ...
            if($result == 0)
            {
               ...
               $this->session->set_flashdata('success', 'Your password reset successfull! Please login with new password');
               redirect('admin');
            }
            else
            {
