...
class Home_visits_list extends NIC_Controller {
  ...
  public function index() 
  {
    $this->validate_login(array('4'));
     ...
     $data['home_visits_total_details'] = $this->home_visit_minor_form_model->home_enquiry_visits_list_details();
     $this->load->view($this->config->item('theme').'reporting/home_visit/home_visit_list_view', $data);
  }

  public function index_old_5_4_24() 
  {
     $this->validate_login(array('4'));
     $login_id = $this->session->userdata('login_id');
     $data['district_details'] = $this->Dashboard_model->district_details($login_id);
     ...
     $data['selected_fields'] = ' DISTINCT ON ("A"."cp_id_fk") "A"."cp_id_fk", "A"."hv_status","A"."sl_no" AS home_visits_sl_no,
                                "A"."cp_type",
                                "A"."cp_id_fk",
                                "A"."incident_id_fk",
                                G.incident_date,
  ...
  public function dateSearch()
  {
    ...
    $data['selected_fields'] = ' DISTINCT ON ("A"."cp_id_fk") "A"."hv_status","A"."sl_no" AS home_visits_sl_no,
                                "A"."cp_type",
                                "A"."cp_id_fk",
                                "A"."incident_id_fk",
                                G.incident_date,
...
$get_data = array(
  ...
  $home_visits_total_details = array();
    $get_data['date_search'] = "G.incident_date BETWEEN '$start_date' AND '$end_date'";
    if($start_date!= '' && $end_date != '')
    {
      $data['home_visits_total_details']=$home_visits_total_details= $this->home_visit_list_model->get_homwvisit_list_dtls($get_data);
      $get_data['date_search'] = "G.incident_date BETWEEN '$start_date' AND '$end_date'";
    }
  ...
  if($start_date!= '' && $end_date != '')
  {
    $get_data['date_search'] = "G.incident_date BETWEEN '$start_date' AND '$end_date'";
     ...
     $data['home_visits_print_details_data'] = $this->home_visit_list_model->get_homwvisit_list_dtls($get_data);
