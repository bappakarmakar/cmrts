...
class Follow_up_visits_list extends NIC_Controller {
 ...
 public function publish_follow_up(){
    $stake_holder_login_id_pk = $this->session->userdata('stake_holder_login_id_pk');
    $sl_no = $this->input->get('sl_no');
    $scheduler_id_fk = $this->input->get('scheduler_id_fk');
    $incident_id_fk = $this->input->get('incident_id_fk');
    $cp_type = $this->input->get('cp_type');

    $sl_no = base64_decode($sl_no);
    $scheduler_id_fk = base64_decode($scheduler_id_fk);
    ...
    $followUpVisitUpdateStatus = $this->follow_up_visit_list_model->follow_up_visits_details_update($update,$where);
    $he_status_update_scheduler=$this->follow_up_visit_list_model->update_scheduler_fu_publish($scheduler_id_fk); // ADDED BY SOUMEN
  ...
  public function delete_followup()
  {
    ...
    $scheduler_update['current_status'] = NULL;
    $schedule_id_where['scheduler_id'] = $scheduler_id_fk;
    
    $default->trans_start();
    $followUpVisitUpdateStatus = $this->follow_up_visit_list_model->follow_up_visits_details_update($update,$where);
    $scheduler_update_status =  $this->follow_up_visit_list_model->follow_up_visits_details_update_scheduler($scheduler_update,$schedule_id_where); // ADDED BY SOUMEN
  ...
  {
      $fileName = 'Register_FollowUp_'.date('Y_m_d');
      ...
      foreach ($follow_up_visits_details as $value){
        ...
        if($value->followup_date != '')
        {
          ...
          $status = null;
          if($value->fv_status==1){$status = 'Saved';}elseif ($value->fv_status==2) {$status = 'Forwarded';}elseif ($value->fv_status==3) {$status = 'Published';}elseif($value->fv_status==4) {$status = 'Reverted';}else{$status = 'Saved As Draft';} 
