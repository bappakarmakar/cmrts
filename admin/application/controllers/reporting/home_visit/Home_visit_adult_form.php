...
class Home_visit_adult_form extends NIC_Controller {
  ...
  public function index($incident_id=null, $cp_type=null, $cp_id=null,$scheduler_id=null) 
  {
    $schd_id = base64_decode($scheduler_id); //changes in routes to get this
  ...
  public function create() {
        ...
        if(empty($all_post_data)){
          ...
          if(count($incidentDetailsIdArray)==4){
            ...
            $scheduler_id = base64_decode($incidentDetailsIdArray[3]); // ADD BY SOUMEN
          }else{
            $incident_id = null;
            $cp_type =  null;
            $cp_id = null;
            $scheduler_id = null; // ADD BY SOUMEN
          }
          ...
          $homeVisitData['scheduler_id_fk'] =  $scheduler_id; // ADD BY SOUMEN
          ...
          $scheduler_status['current_status'] =  $formCompleteStatus; // ADD BY SOUMEN
          ...
          if(count($home_visit_adult_check_status)>0){
            ...
            $home_visit_minor_insert_status = $this->home_visit_minor_form_model->home_visit_minor_update_scheduler($scheduler_status,$scheduler_id); //by soumen
          ...
          if(!empty($Siblings_Details)){
            foreach($Siblings_Details as $key => $value){
              if ( !empty(array_filter($value))) {
                ...
                $SiblingsData['scheduler_id_fk'] =  $scheduler_id; // ADD BY SOUMEN
          ...
          if($home_visit_adult_insert_status>0 && $siblings_status>0 && $siblings_update_delete_status>0){
  ...
  public function siblings_table_record_loade_old(){
    $id = $_GET['id'];
    $idArray = explode("/", $id);
    if(count($idArray)>1){
