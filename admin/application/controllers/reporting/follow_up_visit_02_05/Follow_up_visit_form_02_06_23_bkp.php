  ...
  public function index($incident_id) 
  {
    ...
    if(strtoupper($this->input->server("REQUEST_METHOD") == strtoupper('POST'))){
      if($this->form_validation->run() == TRUE) {
        $this->db->trans_begin();
        $result = $this->follow_up_visit_form_model->insert_follow_up_visit_details($incident_id);
        if($result == 0){
    ...
    $data['incident_details'] = $this->follow_up_visit_form_model->get_incident_details($incident_id);
