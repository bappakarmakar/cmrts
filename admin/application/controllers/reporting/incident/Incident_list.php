...
class Incident_list extends NIC_Controller {
  ...
  public function fusc($incident_id=NULL,$cp_type=NULL,$cp_id_pk=NULL) //SCHEDULER FUNCTION BY SOUMEN
    {
      $login_id = $this->session->userdata('login_id');
      $fu_result['district_details'] = $this->Dashboard_model->district_details($login_id);
      $login_id = $this->session->userdata('login_id');
      $incident_id = base64_decode($incident_id);
      $cp_type = base64_decode($cp_type);
      $cp_id_pk = base64_decode($cp_id_pk);
      $fu_result['cp_type'] = $cp_type;

      $fu_result['result'] =$this->incident_list_model->fu_scheduler($incident_id);
      $fu_result['view'] =$this->incident_list_model->view_scheduler($incident_id,$cp_type);

    $this->load->view($this->config->item('theme').'reporting/incident/incident_follow_up_scheduler',$fu_result);
  }
  public function publish_deo()
  {
    ...
    //code by soumen at 22_03_2025 start
      $result1 = $this->incident_list_model->fu_scheduler($incident_id);
      $row = $result1[0];
      $fu_result = $this->incident_list_model->insert_date_cp1($row);
      if(isset($row->cp_2_id_pk) || !empty($row->cp_2_id_pk)){
        $fu_result1 = $this->incident_list_model->insert_date_cp2($row);
      }
      $update_inc_table = $this->incident_list_model->update_schd_status_inc_table($incident_id);
    //code by soumen at 22_03_2025 end
