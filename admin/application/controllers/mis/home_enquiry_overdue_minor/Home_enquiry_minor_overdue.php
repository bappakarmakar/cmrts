...
class Home_enquiry_minor_overdue extends NIC_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model('mis/Home_enquiry_overdue_minor_model');
  ...
  public function index()
  {
    ...
    $fuv_due_report['data'] = $this->Home_enquiry_overdue_minor_model->get_fuv_due_details($district);
  ...
  public function home_enquiry_minor_overdue_report() // Follow-ups report view 
  {
    ...
    if ($data['force_view'] == 1) {
      $data['report_result'] = $this->Home_enquiry_overdue_minor_model->get_home_enquiry_minor_overdue_details($data);
    }
    $this->load->view($this->config->item('theme') . 'mis/home_enquiry_overdue_minor/Home_enquiry_minor_overdue_view', $data);
  }


  public function home_enquiry_minor_overdue_report_countwise()
  { // Counts details view function
    ...
    if ($data['force_view'] == 1) {
      $data['report_result'] = $this->Home_enquiry_overdue_minor_model->get_home_enquiry_minor_overdue_report_countwise_details($data);
    }

    $this->load->view($this->config->item('theme') . 'mis/home_enquiry_overdue_minor/Home_enquiry_minor_overdue_count_view', $data);
  }
  ...
  {
    $title = "Home Enquiry(minor) Overdue";
    $filename = '';
  ...
  public function excel_download_for_count_details()
  {
    ...
    $filename = "Home_Enquiry(minor)_OverDue_" . $column_name . " " . $title;

    $sheet->getStyle('A1:H1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('ffcc00');
    $sheet->getStyle('A1:H1')->getFont()->setBold(true)->setName('Calibri')->setSize(14);
    ...
    $sheet->setCellValue('G' . $title_line, 'Age at Scheduled Date');
    $sheet->setCellValue('H' . $title_line, 'No. of Days Overdue');
    ...
    foreach ($data['report_result'] as $value) {
      ...
      $sheet->setCellValue('G' . $rows, $age_at_scheduled_date);
      $sheet->setCellValue('H' . $rows, $no_of_days_overdue);
