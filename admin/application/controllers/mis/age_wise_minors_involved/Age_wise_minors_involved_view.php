...
class Age_wise_minors_involved_view extends NIC_Controller {
  ...
  public function District_Wise_Download_Excel($from_date, $to_date)
  {
      ...
      if($this->session->userdata('district')!='')
    {
      ...
      $data['user_dist']="AGE WISE MINORS INVOLVED REPORT DISTRICT NAME - ".$incident_district->district_name;
    }
    else
    {
      $data['user_dist'] = "AGE WISE MINORS INVOLVED REPORT - DISTRICT WISE";
    }
    $fileName = 'MIS_AgeWise_Minors_Districtwise';
    $report_result = $this->Age_wise_minor_involved_model->age_wise_minor_involved_details_district_count($from_date, $to_date);
  ...
  public function SD_Block_Wise_Download_Excel($district_id, $from_date, $to_date)
  {
    ...
    $fileName = 'MIS_AgeWise_ BlockMunicipality_wise';
    $spreadsheet = new Spreadsheet();
    $sheet = $spreadsheet->getActiveSheet();
    ...
    $sheet->setCellValue('A1', 'AGE WISE MINORS INVOLVED REPORT (BLOCK/MUNICIPALITY WISE) FOR '.$data['district']->district_name);
  ...
  public function GP_Ward_Wise_Download_Excel($block_id, $from_date, $to_date)
  {
    ...
    $report_result = $this->Age_wise_minor_involved_model->get_ward_gp_count_details($block_id, $from_date, $to_date);
    $fileName = 'MIS_AgeWise_ WardGP_wise';
    $spreadsheet = new Spreadsheet();
    $sheet = $spreadsheet->getActiveSheet();
    ...
    $sheet->setCellValue('A1', 'AGE WISE MINORS INVOLVED REPORT (GP/WARD WISE) FOR '.$result_block_dist['block_name']);


    $sheet->mergeCells('O1:R1');
    $sheet->setCellValue('O1', 'District Name :'.$result_block_dist['district_name']);
