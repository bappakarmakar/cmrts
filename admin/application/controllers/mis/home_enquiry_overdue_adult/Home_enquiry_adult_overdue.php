...
class Home_enquiry_adult_overdue extends NIC_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model('mis/Home_enquiry_overdue_adult_model');
  ...
  public function index()
  {
    ...
    $fuv_due_report['data'] = $this->Home_enquiry_overdue_adult_model->get_fuv_due_details($district);
  ...
  public function home_enquiry_adult_overdue_report() // Follow-ups report view 
  {
    $data['force_view'] = 0;
    ...
    if ($data['force_view'] == 1) {
      $data['report_result'] = $this->Home_enquiry_overdue_adult_model->get_home_enquiry_adult_overdue_details($data);
    }
    $this->load->view($this->config->item('theme') . 'mis/home_enquiry_overdue_adult/Home_enquiry_adult_overdue_view', $data);
  }


  public function home_enquiry_adult_overdue_report_countwise()
  { // Counts details view function
    ...
    if ($data['force_view'] == 1) {
      $data['report_result'] = $this->Home_enquiry_overdue_adult_model->get_home_enquiry_adult_overdue_report_countwise_details($data);
    }

    $this->load->view($this->config->item('theme') . 'mis/home_enquiry_overdue_adult/Home_enquiry_adult_overdue_count_view', $data);
  }
  ...
  {
    $title = "Home Enquiry(adult) Overdue";
    $filename = '';
  ...
  public function excel_download_for_count_details()
  {
    ...
    $filename = "Home_Enquiry(adult)_OverDue_" . $column_name . " " . $title;
