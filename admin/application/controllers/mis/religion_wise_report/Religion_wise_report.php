...
class Religion_wise_report extends NIC_Controller {
  ...
  public function __construct(){ 
    ...
    $this->load->model('mis/Religion_wise_report_model');
  ...
  public function index()
  {
    ...
    if (strtoupper($this->input->server("REQUEST_METHOD")) == strtoupper('GET'))
    {
        ...
        if(empty($from_date)){
            ...
            {
              $data['field_selection'] = "cmir.block as unique_id,block_location_master_description(cmir.block) as name,";
    ...
    $data['report_result'] = $this->Religion_wise_report_model->get_religion_wise_child_dtls($data);
    // echo "<pre>";print_r($data); echo "</pre>";

    $this->load->view($this->config->item('theme').'mis/religion_wise_report/religion_wise_report_view', $data);
  }
  ...
  public function download_excel()
  {
    ...
    $list_for_name = null;
      ...
      if($data['segregate'] == 'district')
      {
        ...
        $data['group_by'] = $this->district_wise['group_by'];
        $data['order_by'] = $this->district_wise['order_by'];
        $title = "MIS_religion_based_Districtwise";
        //title name 
      ...
      elseif ($data['segregate'] == 'block') 
      {
        ...
        $title = "MIS_religion_based_Blockwise";
      }
      elseif ($data['segregate'] == 'ward_gp') 
      {
        ...
        $title = "MIS_religion_based_WardGP_wise";
        $Identity_Ward_Gp_Block = $this->Master_model->get_ward_gp_block($data['unique_id']);
      ...
      $report_result = $data['report_result'] = $this->Religion_wise_report_model->get_religion_wise_child_dtls($data);
    ...
    foreach ($data['report_result'] as $value)
    {
      ...
      $sheet->setCellValue('E' . $rows, ($value['female_count_hindu'] != 0)?($value['female_count_hindu']):0 );
      $sheet->setCellValue('F' . $rows, ($value['male_count_hindu'] != 0)?($value['male_count_hindu']):0);

      $sheet->setCellValue('G' . $rows, ($value['female_count_muslim'] != 0)?($value['female_count_muslim']):0 );
      $sheet->setCellValue('H' . $rows, ($value['male_count_muslim'] != 0)?($value['male_count_muslim']):0);

      $sheet->setCellValue('I' . $rows, ($value['female_count_sikh'] != 0)?($value['female_count_sikh']):0 );
      $sheet->setCellValue('J' . $rows, ($value['male_count_sikh'] != 0)?($value['male_count_sikh']):0);

      $sheet->setCellValue('K' . $rows, ($value['female_count_buddhist'] != 0)?($value['female_count_buddhist']):0);
      $sheet->setCellValue('L' . $rows, ($value['male_count_buddhist'] != 0)?($value['male_count_buddhist']):0);

      $sheet->setCellValue('M' . $rows, ($value['female_count_christian'] != 0)?($value['female_count_christian']):0 );
      $sheet->setCellValue('N' . $rows, ($value['male_count_christian'] != 0)?($value['male_count_christian']):0);

      $sheet->setCellValue('O' . $rows, ($value['female_count_other'] != 0)?($value['female_count_other']):0 );
      $sheet->setCellValue('P' . $rows, ($value['male_count_other'] != 0)?($value['male_count_other']):0);
    ...
    if($total_row>1)
    {
      ...
      $sheet->setCellValue('E' . $total_row,array_sum(array_column($report_result, 'female_count_hindu')));
      $sheet->setCellValue('F' . $total_row,array_sum(array_column($report_result, 'male_count_hindu')));

      $sheet->setCellValue('G' . $total_row,array_sum(array_column($report_result, 'female_count_muslim')));
      $sheet->setCellValue('H' . $total_row,array_sum(array_column($report_result, 'male_count_muslim')));

      $sheet->setCellValue('I' . $total_row,array_sum(array_column($report_result, 'female_count_sikh')));
      $sheet->setCellValue('J' . $total_row,array_sum(array_column($report_result, 'male_count_sikh')));

      $sheet->setCellValue('K' . $total_row,array_sum(array_column($report_result, 'female_count_buddhis')));
      $sheet->setCellValue('L' . $total_row,array_sum(array_column($report_result, 'male_count_buddhis')));

      $sheet->setCellValue('M' . $total_row,array_sum(array_column($report_result, 'female_count_christian')));
      $sheet->setCellValue('N' . $total_row,array_sum(array_column($report_result, 'male_count_christian')));


      $sheet->setCellValue('O' . $total_row,array_sum(array_column($report_result, 'female_count_other')));
      $sheet->setCellValue('P' . $total_row,array_sum(array_column($report_result, 'male_count_other')));
