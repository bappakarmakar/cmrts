...
class User extends NIC_Controller {
  ...
  public function index() 
  {
     ...
     if($stake_id_fk==3){
        ...
        $deo_user_details = $this->Master_model->get_blck_mnc_by_district($this->session->userdata('district')); //ADDED BY SOUMEN
        ...
        foreach ($deo_user_details as $key => $value) {
          ...
          $newString = str_replace(' ', '-', $district_name);
          $user_name = "CDPO.".$block_name.'.'.$newString;
          $username_check = $this->create_user_form_model->check_duplicate_login_id($user_name);
          // echo "<pre>";print_r($username_check);
          if($username_check != 0){ // if($username_check<0){
            // echo "if";
          }else{
            // echo "else";
            $password =  generateRandomPassword(6);
            $password_hash = hash('sha256',$password);
            $insetUserData[$key]['stake_id_fk'] = 5;
            ...
            $insetUserData[$key]['stake_holder_details'] = 'CDPO';
            $insetUserData[$key]['stake_details_id_fk'] = 5;
