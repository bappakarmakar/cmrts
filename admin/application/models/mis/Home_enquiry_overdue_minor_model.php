...
class Home_enquiry_overdue_minor_model extends CI_Model
{
    ...
    public function get_home_enquiry_minor_overdue_details($data = array())
    {
        ...
        $this->db->where('CAST(schd.fu_names AS INT) =', 0); // 0=Home Enquiry
        $this->db->where('CURRENT_DATE >= schd.calculated_date', null, false); // LESS THAN TODAY DATE
        $this->db->where("cp.cp_dob > cmir.incident_date - INTERVAL '18 years'"); // Filter only minor cp's
    ...
    public function get_home_enquiry_minor_overdue_report_countwise_details($data = array())
    {
