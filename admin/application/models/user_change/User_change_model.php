...
class User_change_model extends CI_Model
{
    ...
    public function get_request_list($data = array())
    {
      ...
      $query = $this->db->select('A.*,B.description,B.style,C.stake_details')
        ...
        // ->where($data);
      if(!empty($data['stake_id_fk_in']))
      {
        $query->where_in('A.stake_id_fk',$data['stake_id_fk_in']);
      }

      if(!empty($data['requested_by']))
      {
        $query->where_in('A.requested_by',$data['requested_by']);
      }
      if(!empty($data['stake_holder_login_id_fk']))
      {
        $query->where_in('A.stake_holder_login_id_fk',$data['stake_holder_login_id_fk']);
      }


      if(!empty($data['requested_district']))
      {
        $query->where('A.requested_district',$data['requested_district']);
      }
      if(!empty($data['requested_subdiv']))
      {
        $query->where('A.requested_subdiv',$data['requested_subdiv']);
      }
      if(!empty($data['requested_block']))
      {
        $query->where('A.requested_block',$data['requested_block']);
      }

      if(!empty($data['requested_subdiv_null']))
      {
        $query->where('A.requested_subdiv IS NULL');
      }
      ...
      {
        $query->where('A.active_status',$data['active_status']);
      }
      if(!empty($data['stake_id_fk_group']))
      {
        $query->group_start();
        if(!empty($data['stake_id_fk_group']['stake_id_fk_in']))
        {
          $query->where_in('A.stake_id_fk',$data['stake_id_fk_group']['stake_id_fk_in']);
        }
        if(!empty($data['stake_id_fk_group']['or_group']))
        {
          $query->or_group_start();
          if(!empty($data['stake_id_fk_group']['or_group']['stake_id_fk_in']))
          {
            $query->where_in('A.stake_id_fk',$data['stake_id_fk_group']['or_group']['stake_id_fk_in']);
          }
          if(!empty($data['stake_id_fk_group']['or_group']['requested_district_null']))
          {
            $query->where('A.requested_district IS NULL');
          }
          $query->group_end();
        }
        $query->group_end();
