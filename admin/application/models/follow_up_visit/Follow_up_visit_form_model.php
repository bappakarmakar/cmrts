...
class Follow_up_visit_form_model extends CI_Model
{
    ...
    public function get_incident_cp_details($cp_id)
    {
         // $query = $this->db->select('cp_gender as gender, cp_age,cp_name,cp_district,cp_block,cp_ward_gp, block_location_master_description(cp_block) AS block_name,district_location_master_description(cp_district) as district_name,cp_phone_no,TO_CHAR(cp_dob, 'DD/MM/YYYY') AS "cp_dob",,cp_age AS inc_cp_age')
            $query = $this->db->select('cp_gender as gender, cp_age, cp_name, cp_district, cp_block, cp_ward_gp, cp_police_station,block_location_master_description(cp_block) AS block_name, district_location_master_description(cp_district) as district_name, cp_phone_no, TO_CHAR(cp_dob, \'DD/MM/YYYY\') AS cp_dob, cp_age AS inc_cp_age')
    ...
    public function follow_up_visit_update_by_sl_no_scheduler($updateData,$sl_no){ // ADD BY SOUMEN
        $default = $this->load->database('default',TRUE);
        $default->where('scheduler_id', $sl_no)
           ->update('cm_follow_up_visit_minor_scheduler',$updateData);
        //print $default->last_query();die();
        return $default->affected_rows();
    }
    public function follow_up_visit_status_insert_scheduler($scheduler_status,$scheduler_id){ // ADD BY SOUMEN
        $default = $this->load->database('default',TRUE);
        //$default->insert('cm_follow_up_visit_minor_scheduler', $scheduler_status);
        $default->where('scheduler_id', $scheduler_id)
           ->update('cm_follow_up_visit_minor_scheduler',$scheduler_status);
        //print $default->last_query();die();
        return $default->affected_rows();
    }
