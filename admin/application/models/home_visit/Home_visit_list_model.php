...
class Home_visit_list_model extends CI_Model
{
    ...
    public function home_enquiry_visits_list_details()
    {
        ...
        if($this->session->userdata('stake_id_fk')==4)
        {
            ...
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk,A.revert_time,A.scheduler_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val FROM cm_incident_report_home_visit AS A LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk WHERE A.active_status=1 AND A.entry_by = $stake_holder_id_fk ORDER BY A.cp_id_fk")->result();
            return $query;
        }
        else if($this->session->userdata('stake_id_fk')==2)
        {
            
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk,A.scheduler_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val FROM cm_incident_report_home_visit AS A LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk WHERE A.active_status=1 AND A.hv_status in(2,3,4) AND B.cp_block = $block  ORDER BY A.cp_id_fk")->result();
    ...
    public function update_scheduler_he_publish($scheduler_id_fk) // ADD BY SOUMEN
    {
    ...
    public function delete_homevisit_details_scheduler($data = array(),$where=array()) // ADD BY SOUMEN
    {
        $this->db->set($data);
        $this->db->where($where);
        $this->db->update('cm_follow_up_visit_minor_scheduler');
