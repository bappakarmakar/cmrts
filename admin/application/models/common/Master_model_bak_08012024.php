...
class Master_model extends CI_Model 
{
	public function get_state_name()
   {
    ...
    {
		if(is_numeric($block_id) && strlen($block_id)<5)
		{
    ...
    {
    	if(is_numeric($state_id) && strlen($state_id)<2)
    	{
    ...
    {
    	if(is_numeric($district_id) && strlen($district_id)<3)
    	{
		   $query = $this->db->select("*")
    ...
    public function get_ward_gp_block($block_id)
    {
    	if(is_numeric($block_id) && strlen($block_id)<5)
    	{
	       $block_details_query = $this->db->select("block_id_pk,block_name, rural_urban")
		   ->from("rp_location_master_block")
		   ->where("block_id_pk", $block_id)
		   ->get()->row();
		   // print_r($block_details_query);die;
