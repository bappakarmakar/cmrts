    ...
    public function incident_list_print_details()
    {
        ...
        if($this->session->userdata('stake_id_fk') == '4' || $this->session->userdata('stake_id_fk') == '2'){
            ...
            if($district != ''){
                ...
                SELECT incident_id_pk FROM cm_incident_report AS cmir
                LEFT JOIN cm_incident_report_contracting_parties AS cmircpo ON cmir.incident_id_pk = cmircpo.incident_id_fk
                WHERE cmir.delete_status = '0' and cmir.created_at is not null and cmir.current_status in(2,3,4))")->result();
             }
        ...
        public function incident_list_print_btwndate_details($start_date,$end_date)
    {
        ...
        if($this->session->userdata('stake_id_fk') == '4' || $this->session->userdata('stake_id_fk') == '2'){
            ...
            if($district != ''){
                ...
                SELECT incident_id_pk FROM cm_incident_report AS cmir
                LEFT JOIN cm_incident_report_contracting_parties AS cmircpo ON cmir.incident_id_pk = cmircpo.incident_id_fk
                WHERE cmir.delete_status = '0' and cmir.created_at is not null and cmir.incident_date between '".$start_date."' and '".$end_date."' and cmir.current_status in(2,3,4))")->result();
             }
        }elseif($this->session->userdata('stake_id_fk') == '6'){
            $query = $this->db->query("select block_master.rural_urban, inc.stake_holder_id_fk, inc.incident_id_pk, inc.incident_date,inc.marriage_date, inc.street_landmark, inc.ward_gp,
            inc.state, inc.district, inc.block, district_location_master_description(inc.district) AS incident_district,
