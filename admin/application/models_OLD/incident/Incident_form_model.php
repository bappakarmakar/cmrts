...
class Incident_form_model extends CI_Model
{
    ...
    // $cp_age_month =  $this->input->post('cp_one_dob');
    ...
    // $data = [
    //     'dob' => $cp_age_month,
    //     'age' => $years,     // Store years in "age" column
    //     'months' => $months, // Store months in "months" column
    //     'days' => $days      // Optionally store days in another column
    // ];
//CODE END BY SOUMEN
        ...
        $uploaded_cp_one_details = array(
            ...
            'created_at' => date('Y-m-d H:i:s'),
            'created_ip' => $_SERVER['REMOTE_ADDR'],
            'cp_age_months' => $this->input->post('cp_one_age_months')
        );
    ...
    // $cp_two_age_month =  $this->input->post('cp_two_dob');

    // $dob_dates = new DateTime($cp_two_age_month);
    // $todays = new DateTime(date('Y-m-d'));
    // $diffs = $today->diff($dob_dates);
    ...
    // $day = $diffs->d;
            ...
            $uploaded_cp_two_details = array(
                ...
                'created_at' => date('Y-m-d H:i:s'),
                'created_ip' => $_SERVER['REMOTE_ADDR'],
                'cp_age_months' => $this->input->post('cp_two_age_months')
            );
        ...
        // --------------------- 01-12-2024 Bappa Start --------------------------------
        if($this->input->post('cp_two_is_available') == '2'){

            //when the CP2 is not available then delete the CP2 row from db
            $this->db->query("DELETE FROM cm_incident_report_contracting_parties WHERE reporting_id = '".$max_child_id."' AND incident_id_fk = '". $last_inst_id."' AND cp_type = '2' ");
    
            }
            // --------------------- 01-12-2024 Bappa END --------------------------------
...
if($this->input->post('cp_two_is_available') == '2'){
    ...
    //when the CP2 is not available then delete the CP2 row from db
        // $reporting_id;
        // $incident_id;
$this->db->query("DELETE FROM cm_incident_report_contracting_parties WHERE reporting_id = '".$reporting_id."' AND incident_id_fk = '". $incident_id."' AND cp_type = '2' ");
