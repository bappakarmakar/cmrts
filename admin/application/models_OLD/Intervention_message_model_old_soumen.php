...
class Intervention_message_model extends CI_Model{
    ...
    public function district_name($district){
      $query = $this->db->query("SELECT district_id_pk, district_name FROM rp_location_master_district WHERE district_id_pk = '".$district."' ")->result_array();
      return $query[0];
    }

    // Intervention Block/Municipality name
    public function block_municipal($district,$block){
      $query = $this->db->query("select * from public.rp_location_master_block where district_id_fk= '".$district."' AND block_id_pk = '".$block."' ")->result_array();
      return $query[0];
    }
    ...
    public function cp_gp_ward($district,$block,$gp_ward_id){
      ...
      $query = $this->db->query("SELECT * FROM rp_location_master_block WHERE district_id_fk= '".$district."' AND block_id_pk = '".$block."' ")->result_array();

      if($query[0]['rural_urban']=='R'){
      ...
      $cp_gp_ward_data = array(
          "block_location_data" => $query[0],
          "gp_ward_query" => $gp_ward_query[0],
        );
    ...
    // CP1 District name
    // public function cp1_district_name($district){
    //   $query = $this->db->query("SELECT district_id_pk, district_name FROM rp_location_master_district WHERE district_id_pk = '".$district."' ")->result_array();
    //   return $query[0];
    // }

    // // CP1 Block/Municipality
    // public function cp1_block_municipal($district,$block){
    //   $query = $this->db->query("select * from public.rp_location_master_block where district_id_fk= '".$district."' AND block_id_pk = '".$block."' ")->result_array();
    //   return $query[0];
    // }
