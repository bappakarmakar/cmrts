...
class Home_visit_list_model extends CI_Model
{
    ...
    public function home_enquiry_visits_list_details()
    {
        ...
        // echo '>>>>'.$stake_holder_id_fk;die;
            ...
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk,A.revert_time, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val,B.cp_dob, A.remarks,A.parents_supported,
                A.family_elders_supported,
                A.peers_supported,
                A.neighbours_supported,
                A.others_supported FROM cm_incident_report_home_visit AS A 
                LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type 
                LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code 
                LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk 
                LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk WHERE A.active_status=1 AND A.entry_by = $stake_holder_id_fk ORDER BY A.cp_id_fk")->result();
        ...
        else if($this->session->userdata('stake_id_fk')==2)
        {
            ...
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val,B.cp_dob 
                ,A.parents_supported,
A.family_elders_supported,
A.peers_supported,
A.neighbours_supported,
A.others_supported
                FROM cm_incident_report_home_visit AS A LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk WHERE A.active_status=1 AND A.hv_status in(2,3,4) AND B.cp_block = $block  ORDER BY A.cp_id_fk")->result();
...
A.others_supported,
A.minor_pregnant,
A.stage_of_pregnancy,
A.kanyashree_id,
A.age_of_home_enquiry,
A.cp_type,
B.cp_age_months,
A.home_enquiry_date,A.revert_reason,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val ,B.cp_dob,  A.publish_time
                    FROM cm_incident_report_home_visit AS A
                     LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk WHERE A.active_status=1 AND A.hv_status in(3) AND B.cp_district = $district  ORDER BY A.cp_id_fk")->result();
            ...
            {
                $query = $this->db->query("select A.mode_of_enquiry,
A.family_income,
A.nutritious_meals,
A.neighbours_community,
A.emergencies,
...
A.cp_type,
B.cp_age_months,
A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val,  A.publish_time,B.cp_dob
                 FROM cm_incident_report_home_visit AS A 
            ...
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_dob,B.cp_gender,B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val FROM cm_incident_report_home_visit AS A LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk LEFT JOIN rp_location_master_block AS block on b.cp_block=block.block_id_pk  WHERE A.active_status=1 AND A.hv_status in(2,3,4) AND block.subdiv_id_fk = $subdiv AND rural_urban='U'  ORDER BY A.cp_id_fk")->result();
    ...
    public function home_enquiry_visits_list_details_by_date($start_date,$end_date)
    {
        ...
        if($this->session->userdata('stake_id_fk')==4)
        {
            ...
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name,A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val ,
                FROM cm_incident_report_home_visit AS A ,B.cp_dob
                LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type 
                LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk WHERE A.active_status=1 AND A.entry_by = $stake_holder_id_fk AND G.incident_date BETWEEN '$start_date' AND '$end_date' AND A.entry_by = $stake_holder_id_fk  ORDER BY A.cp_id_fk")->result();
            return $query;
        }
        else if($this->session->userdata('stake_id_fk')==2)
        {
            
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status,B.cp_dob, H.description as cp_gender_val FROM cm_incident_report_home_visit AS A LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk WHERE A.active_status=1 AND A.hv_status in(2,3,4) AND B.cp_block = $block AND G.incident_date BETWEEN '$start_date' AND '$end_date'  ORDER BY A.cp_id_fk")->result();
...
A.cp_type,
B.cp_age_months,
 A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name,A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val,  A.publish_time FROM 
                    cm_incident_report_home_visit AS A ,B.cp_dob
...
A.stage_of_pregnancy,
A.cp_type,
B.cp_age_months,B.cp_dob,
 A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name,A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_gender, B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val ,  A.publish_time FROM 
                    cm_incident_report_home_visit AS A 
                    LEFT JOIN cm_incident_report_contracting_parties AS B 
                    ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type 
            ...
            $subdiv = $this->session->userdata('subdiv');
            $query = $this->db->query("select A.home_enquiry_date,A.revert_reason,A.age_of_home_enquiry,B.cp_district,district_location_master_description(B.cp_district) AS cp_district_name,B.cp_block,block_location_master_description(B.cp_block) AS cp_block_name, A.cp_id_fk,A.hv_status,A.sl_no AS home_visits_sl_no, A.cp_type, A.cp_id_fk, A.incident_id_fk, G.incident_date, G.reporting_id, B.cp_name, B.cp_age, B.cp_dob, B.cp_gender,B.cp_block, B.cp_ward_gp, F.description as status, H.description as cp_gender_val FROM cm_incident_report_home_visit AS A LEFT JOIN cm_incident_report_contracting_parties AS B ON A.incident_id_fk = B.incident_id_fk AND A.cp_type = B.cp_type LEFT JOIN public.cm_homevisit_status_master AS F ON A.hv_status = F.code LEFT JOIN public.cm_incident_report AS G ON A.incident_id_fk = G.incident_id_pk LEFT JOIN public.cm_gender_master AS H ON B.cp_gender =H.cm_gender_master_id_pk LEFT JOIN rp_location_master_block AS block on b.cp_block=block.block_id_pk  WHERE A.active_status=1 AND A.hv_status in(2,3,4) AND block.subdiv_id_fk = $subdiv AND rural_urban='U' AND G.incident_date BETWEEN '$start_date' AND '$end_date' ORDER BY A.cp_id_fk")->result();
            return $query;
    ...
    public function delete_homevisit_details($data = array(),$where=array())
    {
        $this->db->set($data);
        $this->db->where($where);
        $this->db->update('cm_incident_report_home_visit');
    ...
    public function revertback_homevisit_details($data = array(),$where=array())
    {
        $this->db->set($data);
        $this->db->where($where);
        $this->db->update('cm_incident_report_home_visit');
