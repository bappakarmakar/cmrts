...
<?php //echo '<pre>';print_r($address_changes_details); ?>
<?php 
    $addressChangeMsg = ($address_changes_details->cp_type==1)?'Contracting Party One':'Contracting Party Two';
    $sl_no = ($address_changes_details)?$address_changes_details->sl_no:'';
    ?>
<?php echo form_open('admin/reporting/address_change/address_change_form/edit/'.base64_encode($sl_no), array('class' => 'addressChangeEditForm','name' => 'addressChangeEditForm', 'id' => 'addressChangeEditForm')) ?>
  <input type="hidden" name="sl_no" id="sl_no" value="<?=base64_encode($sl_no)?>">
                     ...
                     <div class="box-body CWC_Second_Row">
                        <div class="card-body CWC_Address_Div">
                           <div class="form-group row">
                             ...
                             <div class="col-sm-6">
                                 <input type="text" class="form-control" id="street_landmark" autocomplete="off" name="street_landmark" placeholder="Street / Landmark" value="<?=($address_changes_details)?$address_changes_details->street_landmark:''?>">
                             </div>
                           </div>
                        </div>
                        <div class="card-body">
                           ...
                           <div class="form-group row">
                             ...
                             <div class="col-sm-6">
                                 <select class="form-control district" name="district" id="district">
                                    ...
                                    <option <?=($address_changes_details->district==$district['district_id_pk'])?'selected':''?>  value="<?php echo $district['district_id_pk'];?>"><?php echo $district['district_name'];?></option>
                                    <?php } ?>                     
                                 </select>
                           ...
                           <div class="form-group row">
                             ...
                             <div class="col-sm-6">
                                 <select class="form-control" name="block" id="block">
                                    ...
                                    <option <?=($address_changes_details->block==$block_val['block_id_pk'])?'selected':''?> value="<?php echo $block_val['block_id_pk'];?>"><?php echo $block_val['block_name'];?></option> 
                                    <?php } ?>
                                 </select>
                                    ...
                                    <?php if(!empty($Ward_Gp_Block)){?>
                                       <?php if($Ward_Gp_Block->rural_urban == 'U'){?>
                                         <?php foreach($cp_ward as $Incident_Ward_Value){ ?>
                                           <option <?=($address_changes_details->ward_gp==$Incident_Ward_Value['ward_id_pk'])?'selected':''?> value="<?php echo $Incident_Ward_Value['ward_id_pk'];?>"><?php echo $Incident_Ward_Value['ward_no'];?></option> 
                                         <?php } ?>
                                       <?php }else{?>
                                         <?php foreach($cp_gp as $Incident_GP_Value){ ?>
                                            <option <?=($address_changes_details->ward_gp==$Incident_GP_Value['gp_id_pk'])?'selected':''?> value="<?php echo $Incident_GP_Value['gp_id_pk'];?>"><?php echo $Incident_GP_Value['gp_name'];?></option> 
                                         <?php } ?>
                                    <?php } } ?>
                                 </select>
                                 <?php echo form_error('block'); ?>
                        ...
                        <div class="card-body CWC_Address_Div">
                           <div class="form-group row">
                             ...
                             <div class="col-sm-6">
                                 <input type="text" class="form-control pin_code_validate" id="pin_code" autocomplete="off" name="pin_code" placeholder="Pin Code" value="<?=$address_changes_details->pin_code;?>">
                        ...
                        <div class="card-body CWC_Address_Div">
                           <div class="form-group row">
                             ...
                             <div class="col-sm-6">
                                 <input type="text" class="form-control" id="police_station" autocomplete="off" name="police_station" placeholder="Police Station" value="<?=$address_changes_details->police_station;?>">
                        ...
                        <div class="card-body CWC_Address_Div">
                           <div class="form-group row">
                             ...
                             <div class="col-sm-6">
                                 <textarea rows="3" name="address" id="address" class="form-control" placeholder="Address"><?=$address_changes_details->address;?></textarea>
                        ...
                        <div class="card-body">
                           <div class="form-group row">
                             ...
                             <div class="col-sm-6">
                                 <textarea rows="3" name="remarks" class="form-control" placeholder="Remarks"><?=$address_changes_details->remarks;?></textarea>
...
$(document).ready(function(){
      $(document).off('submit', '#addressChangeEditForm').on('submit', '#addressChangeEditForm',function (event) {
         event.preventDefault();
         var csrf_token_value = $('input[name=csrf_cmrts]').val();
         var formName = 'addressChangeEditForm';
         var formData = new FormData($('form[name="' + formName + '"]')[0]);
         formData.append("csrf_cmrts", csrf_token_value);
         formData.append("action", "submit");
         $.ajax({
            type: 'POST',
             url: '<?php echo base_url()?>admin/reporting/address_change/Address_change_form/edit_from_update/',
             data: formData,
             processData: false,
             cache: false,
             contentType: false,
             ...
             success: function (data){
               ...
               if (data.formCompleteStatus) {
                  ...
                  swal({
                   title: "The Address Change Report has been saved in the Register",
                   type: "warning",
                  ...
                  swal({
                     ...
                     confirmButtonClass: "btn-success",
                     confirmButtonText: "Save as Draft and return to Address Change List",
                     cancelButtonClass: "btn-primary",
                     cancelButtonText: "Save as draft and close Address Change Report",
                     closeOnConfirm: false,
                     closeOnCancel: false
                    }, function(isConfirm) {
                      if (isConfirm) {
                           ...
                           type: 'POST',
                            ...
                            success: function (data){
                              ...
                              if (data.draftErrorFields.length == 0) {
                                 window.location.href = 'admin/reporting/address/address_list/';
                              }else{
                               swal.close();  
                              }
                            }
