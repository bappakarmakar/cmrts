                  ...
                  <div class="card-body p-0">
                     ...
                     <div class="box-body CWC_Second_Row">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6">
                                <input type="text" name="cp_two_pin_code" id="cp_two_pin_code" class="form-control cp_two_pin_code_validate pin_code_validate" value="<?php echo set_value('cp_two_pin_code'); ?>" placeholder="Pin Code" maxlength="6" onpaste="return false">
                                <span id="lbl_error_cp_two_pin_code" style="color: red;"></span>
        ...
        if(fetch_entered_address == undefined){
            $.ajax({
              ...
              success: function(response)
              {
                ...
                $("#cp_two_police_station").val('').prop("readonly", false);
                $("#cp_two_pin_code").val('').prop("readonly", false);
            ...
            $.ajax({
              url:'<?php echo base_url()?>admin/reporting/address/Address_list/Fetch_Edit_CP_Two_Entered_Address',
              type:'GET',
              ...
              success: function(response)
              {
                ...
                var cp_two_block = response.block;
                // var cp_two_district = response.cp_two_district;
                var cp_two_age = response.age;

                var street_landmark = response.street_landmark;
                var cp_two_police_station = response.police_station;
                var cp_two_pin_code = response.pin_code;
                var cp_two_ward_gp = response.ward_gp;
                var cp_two_ward_gp_id = response.cp_two_ward_gp_id;
                var cp_two_ward_gp_name = response.cp_two_ward_gp_name;
                var address = response.address;
                var remarks = response.remarks;


                // alert(cp_one_ward_gp);
                // alert(cp_two_ward_gp_name);
                $("#street_landmark").val(street_landmark).prop("readonly", true); 
                $("#cp_two_police_station").val(cp_two_police_station).prop("readonly", true); 
                $("#cp_two_pin_code").val(cp_two_pin_code).prop("readonly", true);
                 $("#address").val(address).prop("readonly", true);
                $("#remarks").val(remarks).prop("readonly", true);

                $('#district').html(`<option value="${district_id}">${district}</option>`).css({"cursor":"not-allowed"}).attr("readonly", true);
                $('#block').html(`<option value="${block_id}">${cp_two_block}</option>`).css({"cursor":"not-allowed"}).attr("readonly", true);
                $('#ward_gp').html(`<option value="${cp_two_ward_gp_id}">${cp_two_ward_gp_name}</option>`).css({"cursor":"not-allowed"}).attr("readonly", true);
                // if(cp_two_age >= 18 ){
