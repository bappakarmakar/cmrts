         ...
         <div class="box-body">
            <table class="table table-bordered table-hover" id="mytable">
                <div class="user_box">
                    ...
                    <?php } ?>
                    <?php if($this->session->userdata('stake_id_fk') == '6'){?>
                        <a href="<?php echo base_url()?>admin/user_list/user/downlod_excel_sdo_level_deo" class="btn btn-success" style="margin-left: 12px;"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Download Excel</a>
                    <?php } ?>
                    <?php if($this->session->userdata('stake_id_fk') == '1'){?>
                        <a href="<?php echo base_url()?>admin/user_list/user/export_all_users" class="btn btn-success"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export Users</a>
                    <?php } ?>
                </div>
               <thead>
               ...
               <tbody id="childAppend">
                  ...
                  <tr>
                     ...
                     <?php }else{?>
                           <a href="javascript:void()" class="btn btn-danger" onClick="Activate_User('<?php echo $value['stake_holder_login_id_pk']; ?>')"><i class="fa fa-times" aria-hidden="true"></i> Deactive</a>
                     <?php } } ?>
                     </td>
                  </tr>
                  <?php } ?>
 ...
 function(isConfirm){
   if(isConfirm){
       ...
       $.ajax({
           ...
           {
               swal("Activated", "Activated to user", "success");
               setTimeout(function(){
       ...
       swal("Cancelled", "Activated cancel to user!", "error");
       setTimeout(function(){
          window.location.reload();
       }, 1500);
   } 
 ...
 function(isConfirm){
   if(isConfirm){
       ...
       $.ajax({
           ...
           success:function(response)
           {
               swal("Deactivate", "Deactivate to user", "success");
               setTimeout(function(){
                  window.location.reload();
               }, 2000);
           }
