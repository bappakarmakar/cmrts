                ...
                <table class="table table-bordered table-hover" id="mytable">
                   ...
                   <tbody id="childAppend">
                      ...
                      foreach($police_case_details as $value){
                        ...
                        if($value->current_status=='22'){
                          $color = "background-color:#eff7f4";
                        }else{
                          $color = "background-color:#f7efef";
                        }
                      ?>
...
<script>
  ...
  $('#exportExcel').on('click', function () {
    ...
    // Get header labels
    var headers = [];
    $('#mytable thead tr:last-child th').each(function () {
      headers.push(stripHtml($(this).html().trim()));
    });

    // Get all row data (from all pages)
    var allData = [];
    allData.push(headers); // Add headers to top
    ...
    table.rows({ search: 'applied' }).every(function () {
      ...
      $(row).find('td').each(function () {
        var cellText = stripHtml($(this).html().trim());
    ...
    XLSX.writeFile(wb, "Police_Case_Data.xlsx");
  });
</script>
